...
<body background="img/background.jpg">
	...
	<form action="${pageContext.request.contextPath }/pmanager?method=modifyMemberTask&taskId=${modifyTask.taskId}" method="post" >
		<table border="1" class="table table-hover">
				...
				<td>所属项目
					<select name="projectId" class="form-control">
						<c:forEach var="projectList" items="${projectList}">
							<c:choose>
								<c:when test="${modifyTask.projectId eq projectList.projectId}">
									<option selected="selected" value="${projectList.projectId}">${projectList.projectName}</option>
								</c:when>
								<c:otherwise>
			...
			<tr>
				<td colspan="1"><a>员工任务</a>
					<input type="text"  class="form-control" name="taskName"  value="${modifyTask.taskName}"/>
				</td>
				<td colspan="1"><a>任务进度</a>
					<input readonly="readonly" name="taskProgress"  value="${modifyTask.taskProgress}"/>
				</td>
			</tr>
			<tr>
